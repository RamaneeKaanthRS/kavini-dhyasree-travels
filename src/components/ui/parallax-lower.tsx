...
const defaultLayers: ParallaxLayer[] = [
  {
    src: 'https://images.unsplash.com/photo-1494548162494-384bba4ab999?q=80&w=3200&auto=format&fit=crop', // Sunset image
    alt: 'background',
    speedX: 0.03,
    speedY: 0.038,
    speedZ: 0,
...
export const ParallaxHero: React.FC<ParallaxHeroProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        'relative h-[80vh] min-h-[600px] w-full overflow-hidden bg-gradient-to-b from-orange-950 via-rose-900 to-orange-800',
        className
      )}
    >
      <div className="absolute inset-0 z-[100] pointer-events-none bg-[radial-gradient(ellipse_at_center,rgba(0,0,0,0)_50%,rgba(0,0,0,0.8))]" />
      ...
      {layers.map((layer, index) => (
        <img
          ...
          alt={layer.alt}
          className={cn(
            'absolute pointer-events-none transition-transform duration-[450ms] ease-out object-cover',
            index !== 0 && 'sepia-[.4] hue-rotate-[-15deg] saturate-[1.5] brightness-75 contrast-125',
            layer.className
          )}
          style={{
            width: layer.width,
      ...
      <div
        ref={textRef}
        className="absolute z-[999] text-white text-center pointer-events-auto transition-transform duration-[450ms] ease-out w-full max-w-7xl px-4 sm:px-6 lg:px-8 flex items-center justify-center"
        style={{
          top: '50%',
          left: '50%',
          transform: 'translate(-50%, -50%)',
